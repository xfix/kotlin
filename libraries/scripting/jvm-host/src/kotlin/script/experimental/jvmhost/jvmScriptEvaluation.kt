...
open class BasicJvmScriptEvaluator : ScriptEvaluator {
    ...
    private fun getConfigurationWithClassloader(
        ...
        if (baseConfiguration.containsKey(ScriptEvaluationConfiguration.jvm.actualClassLoader))
            ...
            val module = (script as? KJvmCompiledScript<*>)?.compiledModule
                ?: throw IllegalArgumentException("Unexpected compiled script type: $script")

            val baseClassLoader = baseConfiguration[ScriptEvaluationConfiguration.jvm.baseClassLoader]
            val classLoaderWithDeps =
                if (baseConfiguration[ScriptEvaluationConfiguration.jvm.loadDependencies] == false) baseClassLoader
                else script.makeClassLoaderFromDependencies(baseClassLoader)
            val classloader = module.createClassLoader(classLoaderWithDeps)
