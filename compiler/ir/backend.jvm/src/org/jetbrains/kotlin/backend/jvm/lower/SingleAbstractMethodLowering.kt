    ...
    override fun lower(irClass: IrClass) {
        ...
        irClass.transformChildrenVoid(object : IrElementTransformerVoidWithContext() {
            ...
            private fun implement(
                ...
                subclass.declarations += fields
                subclass.declarations += buildConstructor {
                    ...
                    parent = subclass
                    valueParameters += fields.mapIndexed { i, field ->
                        buildValueParameter {
                            name = field.name
                            type = field.type
                            index = i
                ...
                subclass.declarations += buildFun {
            ...
            private fun createObjectProxy(superType: IrType, invokableType: IrType): IrClass =
                implement(superType, listOf(invokableType)) { fields ->
                    context.createIrBuilder(symbol).irBlockBody(startOffset, endOffset) {
                        ...
                        +irReturn(irCall(invokableClass.functions.single { it.name.toString() == "invoke" }).apply {
                ...
                implement(superType, reference.arguments.mapNotNull { it?.type }) { fields ->
                    context.createIrBuilder(symbol).irBlockBody(startOffset, endOffset) {
                        +irReturn(irCall(reference.symbol).apply {
                            ...
                            dispatchReceiver = reference.symbol.owner.dispatchReceiverParameter?.let { arguments.removeAt(0) }
                            extensionReceiver = reference.symbol.owner.extensionReceiverParameter?.let { arguments.removeAt(0) }
