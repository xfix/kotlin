...
class KotlinMPPGradleModelBuilder : ModelBuilderService {
    ...
    private fun buildDependencies(
        ...
        val dependencyTransformations = buildMetadataDependencyTransformations(dependencyHolder, configurationName)
    ...
    private fun buildMetadataDependencyTransformations(
        kotlinSourceSet: Any,
        configurationName: String
    ): Iterable<KotlinMetadataDependencyTransformation> {
        val getTransformations = kotlinSourceSet.javaClass.getMethodOrNull("getDependenciesTransformation", String::class.java)
            ?: return emptyList()
        val transformations = getTransformations(kotlinSourceSet, configurationName) as Iterable<*>
    ...
    private class DependencyAdjuster(
        ...
        dependencyTransformations: Iterable<KotlinMetadataDependencyTransformation>
    ) {
        private val adjustmentMap = HashMap<ExternalDependency, List<ExternalDependency>>()

        private val libraryDependencyTransformation =
            dependencyTransformations.filter { it.projectPath == null }.associateBy { it.groupId to it.moduleName }

        private val projectDependencyTransformation =
            dependencyTransformations.filter { it.projectPath != null }.associateBy { it.projectPath }
        ...
        private fun adjustLibraryDependency(dependency: ExternalDependency, parentScope: String? = null): List<ExternalDependency> =
            when (dependency) {
                is ExternalLibraryDependency -> {
                    ...
                    when {
                        replaceFiles != null -> replaceFiles.mapNotNull { (sourceSetName, replaceFiles) ->
                            DefaultExternalLibraryDependency(dependency).apply {
                                ...
                                file = replaceFiles.first() //TODO support multi-file dependencies
        ...
        fun adjustDependency(dependency: ExternalDependency, parentScope: String? = null): List<ExternalDependency> {
            return adjustmentMap.getOrPut(dependency) {
                ...
                val commonDependencies = if (dependencyProject.path in projectDependencyTransformation) {
                    val visibleSourceSets = projectDependencyTransformation.getValue(dependencyProject.path).useFilesForSourceSets.keys
